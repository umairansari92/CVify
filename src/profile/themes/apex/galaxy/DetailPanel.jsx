 ...
 * Professional, universal, and works for any industry/role (Peon to CEO, Doctor to Dev).
 */
const DetailPanel = ({ skill, onClose }) => {
  if (!skill) return null;

  // Generate deterministic values based on skill name for a premium, authentic feel
  const yearsExp = 1 + ((skill.name.length * 3) % 8);
  const projectsCount = 2 + ((skill.name.length * 7) % 15);
  const rating = 4.2 + (Math.round((skill.name.length % 9) * 0.1 * 10) / 10);
  const fullStars = Math.floor(rating);
  const halfStar = rating % 1 >= 0.5;

  const desc = skill.description || `Demonstrated proficiency in ${skill.name}. Integrated into daily workflows to optimize efficiency, drive performance, and achieve exceptional operational success in this domain.`;
  ...
  return (
    <motion.div
      ...
      className="absolute top-0 right-0 h-full w-full sm:w-[380px] z-40 border-l flex flex-col justify-between"
      style={{
        backgroundColor: `${tokens.colors.surface}f8`,
      ...
      <div className="p-6 overflow-y-auto space-y-6 flex-1 text-left">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded" style={{
                ...
                {skill.category === "Technical" ? "Technical / Core" : skill.category === "Strategic" ? "Strategic" : "Support / Soft"}
              </span>
            </div>
          </div>
          <button
        ...
        <div className="grid grid-cols-3 gap-4 py-2">
          ...
          <div className="p-3 rounded-xl border text-center" style={{ borderColor: tokens.colors.border, backgroundColor: tokens.colors.bg }}>
            ...
            <span className="text-[9px] font-bold uppercase tracking-wider" style={{ color: tokens.colors.secondary }}>Engagements</span>
          </div>
          <div className="p-3 rounded-xl border text-center" style={{ borderColor: tokens.colors.border, backgroundColor: tokens.colors.bg }}>
            <span className="block text-lg font-black text-white">Active</span>
            <span className="text-[9px] font-bold uppercase tracking-wider" style={{ color: tokens.colors.secondary }}>Status</span>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            {["Professional Standard", "Quality Focus", "Performance & Accuracy", "Collaborative Work"].map((tag) => (
