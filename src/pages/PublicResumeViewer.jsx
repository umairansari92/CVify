...
const ErrorPage = ({ message }) => (
  <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-8 text-center">
    <Helmet>
      <title>Resume Not Found | CVify Pro</title>
    ...
    <p className="text-slate-400 text-sm max-w-sm mb-8 leading-relaxed">
      This link may have been disabled or regenerated by its owner.
    </p>
    <Link
      to="/"
      className="px-8 py-3 bg-primary text-white rounded-xl text-sm font-bold tracking-wide
                 shadow-[0_0_24px_rgba(59,130,246,0.3)] hover:brightness-110 transition-all"
    >
      Build Your Resume on CVify Pro →
    </Link>
  </div>
);
...
const PublicResumeViewer = () => {
  ...
  // ── Error ─────────────────────────────────────────────────────────────────
  if (error || !resume) {
    return <ErrorPage message={error || "Resume not found."} />;
  }
