    ...
    <div className="bg-[var(--surface)] border border-[var(--border)] rounded-3xl p-6 sm:p-8 space-y-6 shadow-sm text-[var(--text-primary)]">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-[var(--border)] pb-5">
        <div>
          ...
          <p className="text-[var(--text-secondary)] text-xs mt-1">
            Rewritten using Google's XYZ Formula: <strong className="text-[var(--text-primary)]">Accomplished [X] as measured by [Y] by doing [Z]</strong>.
          </p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {bulletFixes.map((item, idx) => {
          ...
          return (
            <div key={idx} className="bg-[var(--surface-muted)] p-5 rounded-2xl border border-[var(--border)] space-y-4">
              <div className="flex items-center justify-between gap-2 text-xs">
                <span className="font-bold text-[var(--text-primary)]">
                  Transformation #{idx + 1}: <span className="text-[var(--text-secondary)] font-normal">{critiqueText}</span>
                </span>
                <span className="text-[10px] font-bold px-2 py-0.5 rounded border text-amber-500 bg-amber-500/10 border-amber-500/20 shrink-0">
                  Google XYZ Formula
                </span>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs font-mono">
                ...
                <div className="p-4 rounded-xl bg-red-500/10 border border-red-500/20 space-y-1.5">
                  <span className="text-red-500 dark:text-red-400 text-[10px] font-bold uppercase font-sans tracking-wider block">
                ...
                {/* After */}
                <div className="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/30 space-y-2 relative group">
                  <div className="flex items-center justify-between">
                    <span className="text-emerald-600 dark:text-emerald-400 text-[10px] font-bold uppercase font-sans tracking-wider flex items-center gap-1">
                      <Sparkles className="w-3 h-3" /> Recommended ATS Fix
                    </span>
                    <button
                      onClick={() => handleCopy(improvedText, idx)}
                      className="px-2.5 py-1 rounded-lg bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-700 dark:text-emerald-300 text-[10px] font-bold font-sans flex items-center gap-1 transition-all"
                    >
                      {copiedIdx === idx ? <Check className="w-3 h-3" /> : <Copy className="w-3 h-3" />}
                      {copiedIdx === idx ? "Copied!" : "Copy"}
                    </button>
                  </div>
                  <p className="text-emerald-700 dark:text-emerald-200 leading-relaxed font-sans text-xs font-semibold">
                    "{improvedText}"
                  </p>
                </div>
              </div>
