...
const Contact = ({ user, contactForm, setContactForm, handleContactSubmit, isSending }) => {
  ...
  if (!email && !socialLinks.github && !socialLinks.linkedin) return null;

  return (
    <section
      id="contact"
      className="relative z-10 px-6 md:px-12 py-32 md:py-48 border-t"
      ...
      <div className="mx-auto max-w-[1400px] flex flex-col items-center text-center">
        ...
        {email && (
          ...
          >
            ...
            >
              {/* Hover fill */}
              <span
                className="absolute inset-0 w-full h-full -translate-y-full group-hover:translate-y-0 transition-transform duration-500"
                style={{ backgroundColor: tokens.colors.primary }}
              />
              <span
                className="relative z-10 text-sm md:text-base font-bold uppercase tracking-widest transition-colors duration-500 group-hover:text-black"
        ...
        {(socialLinks.github || socialLinks.linkedin || socialLinks.twitter || socialLinks.website) && (
          <motion.div
            ...
            className="mt-20 flex items-center justify-center flex-wrap gap-8"
          >
            {socialLinks.github && (
              <a
                ...
                className="uppercase text-xs font-bold tracking-widest transition-colors hover:text-[var(--primary)]"
