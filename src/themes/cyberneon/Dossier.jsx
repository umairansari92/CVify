    ...
    <section className="relative py-20 md:py-28 px-6 md:px-16 border-t" style={{ borderColor: tokens.colors.cardBorder, backgroundColor: tokens.colors.background }}>
      <div className="max-w-6xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="mb-12 text-center">
          <p className="text-xs uppercase tracking-[0.4em] mb-3" style={{ color: tokens.colors.primary, fontFamily: tokens.fonts.mono }}>&gt; CREDENTIAL_DOSSIER</p>
          <h2 className="text-4xl md:text-5xl font-bold uppercase" style={{ fontFamily: tokens.fonts.heading, color: '#CDD6F4' }}>The Dossier</h2>
        </motion.div>
        ...
        {achievements.length === 0 && languages.length === 0 && isOwner ? (
          <div className="text-center py-14 border border-dashed rounded-2xl" style={{ borderColor: tokens.colors.cardBorder }}>
            <p className="text-sm font-mono mb-6 opacity-60" style={{ fontFamily: tokens.fonts.mono, color: tokens.colors.textMuted }}>&gt; NO_DOSSIER_DATA</p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
            {achievements.length > 0 && (
              <div className={languages.length > 0 ? "lg:col-span-8" : "lg:col-span-12"}>
                <h4 className="text-xs uppercase tracking-[0.3em] mb-8 pb-3 border-b" style={{ fontFamily: tokens.fonts.mono, borderColor: tokens.colors.cardBorder, color: tokens.colors.primary }}>
                  &gt; HONORS_AND_AWARDS
                </h4>
                <div className="space-y-10">
                  {achievements.map((award, idx) => (
                    <motion.div key={idx} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: idx * 0.1 }}
                      className="relative pl-6 border-l-2 transition-colors duration-300" style={{ borderColor: tokens.colors.cardBorder }}
                      onMouseEnter={(e) => e.currentTarget.style.borderColor = tokens.colors.primary}
                      onMouseLeave={(e) => e.currentTarget.style.borderColor = tokens.colors.cardBorder}
                    >
                      <div className="flex flex-col md:flex-row md:items-baseline justify-between gap-2 mb-2">
                        <h5 className="text-xl font-bold" style={{ color: '#CDD6F4', fontFamily: tokens.fonts.heading }}>
                        ...
                        <span className="text-[10px] uppercase tracking-widest px-2 py-1 rounded" style={{ fontFamily: tokens.fonts.mono, color: tokens.colors.primary, backgroundColor: `${tokens.colors.primary}11` }}>{award.date}</span>
                      </div>
                      <p className="text-sm leading-relaxed" style={{ color: tokens.colors.textMuted, fontFamily: tokens.fonts.body }}>
                ...
                <div className="space-y-4">
                  {languages.map((lang, idx) => (
                    <motion.div key={idx} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: idx * 0.1 }}
                      className="p-5 rounded-xl border flex items-center justify-between transition-all" style={{ backgroundColor: tokens.colors.cardBg, borderColor: tokens.colors.cardBorder }}
                      onMouseEnter={(e) => { e.currentTarget.style.borderColor = tokens.colors.primary; e.currentTarget.style.boxShadow = `0 0 15px ${tokens.colors.primary}22`; }}
                      onMouseLeave={(e) => { e.currentTarget.style.borderColor = tokens.colors.cardBorder; e.currentTarget.style.boxShadow = 'none'; }}
                    >
                      <span className="text-sm uppercase tracking-widest font-bold" style={{ color: '#CDD6F4', fontFamily: tokens.fonts.mono }}>
                        <InlineEdit isOwner={isOwner} value={lang.name} onSave={(v) => handleArrayUpdate("languages", idx, { ...lang, name: v })}>{lang.name}</InlineEdit>
                      </span>
                      <span className="text-[10px] uppercase tracking-widest px-3 py-1 rounded-full" style={{ backgroundColor: `${tokens.colors.primary}11`, color: tokens.colors.primary, fontFamily: tokens.fonts.mono }}>{lang.proficiency}</span>
                    </motion.div>
                  ))}
                </div>
              </div>
