    ...
    <section className="relative py-20 md:py-28 px-6 md:px-16 border-t text-center" style={{ borderColor: tokens.colors.cardBorder, backgroundColor: tokens.colors.background }}>
      <div className="max-w-4xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="mb-16">
          <p className="text-xs uppercase tracking-[0.4em] mb-3" style={{ color: tokens.colors.primary, fontFamily: tokens.fonts.mono }}>&gt; TRUST_SIGNALS</p>
          <h2 className="text-4xl md:text-5xl font-bold uppercase" style={{ fontFamily: tokens.fonts.heading, color: '#CDD6F4' }}>Client Feedback</h2>
        </motion.div>
        ...
        {testimonials.length === 0 && isOwner ? (
          <div className="text-center py-14 border border-dashed rounded-2xl" style={{ borderColor: tokens.colors.cardBorder }}>
            <p className="text-sm font-mono mb-6 opacity-60" style={{ fontFamily: tokens.fonts.mono, color: tokens.colors.textMuted }}>&gt; NO_TESTIMONIALS_FOUND</p>
          ...
          <div className="space-y-24">
            {testimonials.map((t, index) => (
              <motion.div key={index} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="flex flex-col items-center">
                <div className="text-8xl leading-none mb-4 opacity-30 pointer-events-none select-none" style={{ color: tokens.colors.primary, fontFamily: tokens.fonts.heading, textShadow: `0 0 30px ${tokens.colors.primary}88` }}>"</div>
                <blockquote className="text-xl md:text-2xl leading-relaxed mb-8 italic" style={{ fontFamily: tokens.fonts.body, color: '#CDD6F4', fontWeight: 300 }}>
                ...
                <div className="flex flex-col items-center gap-1">
                  <span className="font-bold uppercase tracking-widest text-sm" style={{ color: tokens.colors.primary, fontFamily: tokens.fonts.mono }}>
